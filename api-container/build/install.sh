...
Now you should access CLI script: $ api-container --help
Alternatively you can access CLI script via poetry run: $ poetry run api-container --help
To deactivate virtualenv simply type: $ deactivate
To activate shell completion:
 - for bash: $ echo 'eval "$(_API_CONTAINER_COMPLETE=source_bash api-container)' >> ~/.bashrc
 - for zsh: $ echo 'eval "$(_API_CONTAINER_COMPLETE=source_zsh api-container)' >> ~/.zshrc
 - for fish: $ echo 'eval "$(_API_CONTAINER_COMPLETE=source_fish api-container)' >> ~/.config/fish/completions/api-container.fish
EOF