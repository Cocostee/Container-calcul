...
minikube image load "api-container:${TAG}"

echo "[dev-env] creating api-container namespace"
kubectl create namespace api-container

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl get namespace api-container -n api-container"
...
kubectl create -f manifests/persistent-storage-no-pvc-deletion.yaml -n api-container

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl rollout status deployment.apps/rfs-redisfailover-persistent-keep -n api-container"
until $ROLLOUT_STATUS_CMD || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done
...
STATEFULSET_REPLICAS=$(kubectl get statefulset rfr-redisfailover-persistent-keep -o jsonpath='{.spec.replicas}' -n api-container)
ATTEMPTS=0
until [[ ${STATEFULSET_REPLICAS} -eq $(kubectl get statefulset rfr-redisfailover-persistent-keep -o jsonpath='{.status.readyReplicas}' -n api-container) ]] || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done

echo "[dev-env] installing api-container charts"
helm upgrade --install \
    api-container charts/api-container \
    --namespace api-container \
    --set ingress.host.name="api-container.${INGRESS_HOST}"

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl rollout status deployment/api-container -n api-container"
until $ROLLOUT_STATUS_CMD || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done
...
FastAPI available under: http://api-container.${INGRESS_HOST}/
