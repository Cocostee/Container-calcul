...
.. code-block:: bash
    ...
    $ api-container --help
    Usage: api-container [OPTIONS] COMMAND [ARGS]...

      Api-container CLI root.
    ...
    Commands:
      serve  api-container CLI serve command.
    $ api-container serve --help
    Usage: api-container serve [OPTIONS]
...
.. code-block:: bash
    ...
    $ make image
    $ podman run -dit --name api-container -p 8000:8000 api-container:$(cat TAG)
    f41e5fa7ffd512aea8f1aad1c12157bf1e66f961aeb707f51993e9ac343f7a4b
    $ podman ps
    CONTAINER ID  IMAGE                                 COMMAND               CREATED        STATUS            PORTS                   NAMES
    f41e5fa7ffd5  localhost/api-container:0.1.0  /usr/bin/fastapi ...  2 seconds ago  Up 3 seconds ago  0.0.0.0:8000->8000/tcp  api-container
    $ curl localhost:8000/api/ready
    {"status":"ok"}
...
You can implement your own web routes logic straight away in ``api_container.controllers`` submodule. For more information please see `FastAPI documentation <https://fastapi.tiangolo.com/tutorial/>`__.
...
.. code-block:: bash
    ...
    Usage:
      ...
      clean-image      Clean api-container image
      install          Install api-container with poetry
      metrics          Run api-container metrics checks
      unit-test        Run api-container unit tests
      integration-test  Run api-container integration tests
      docs             Build api-container documentation
...
* RedisClient ``api_container.app.utils.redis``
* AiohttpClient ``api_container.app.utils.aiohttp_client``
...
.. literalinclude:: ../api_container/app/exceptions/http.py
    :language: python
...
All application configuration is available in ``api_container.config`` submodule.
...
.. literalinclude:: ../api_container/config/gunicorn.py
    :language: python

Routes
~~~~~~

Endpoints are defined in ``api_container.app.router`` submodule. Just simply import your controller and include it to FastAPI router:

.. literalinclude:: ../api_container/app/router.py
    :language: python
