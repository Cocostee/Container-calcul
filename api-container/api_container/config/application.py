...
class Application(BaseSettings):
    ...
    DEBUG: bool = True
    PROJECT_NAME: str = "api-container"
    VERSION: str = __version__
    DOCS_URL: str = "/"
    USE_REDIS: bool = False
    # Database connection string. Read from the DATABASE_URL environment
    # variable (no FASTAPI_ prefix) to match the docker-compose configuration.
    DATABASE_URL: str = Field(
        default="postgresql://app_user:app_password@db:5432/container_app",
        env="DATABASE_URL",
    )
    # Front-end origins allowed to call the API (browser CORS).
